...
export default function handler(req, res) {
  ...
  if (!/^\d{1,12}$/.test(serialText)) {
    return res.status(400).json({
      error: "El numero de serie debe tener solo numeros (maximo 12 digitos)."
    });
  }

  if (!Number.isInteger(serial) || serial <= 0) {
    return res.status(400).json({ error: "El numero de serie debe ser mayor a cero." });
  }
  ...
  if (isInvalid) {
    return res.json({
      valid: false,
      message: `Serie NO valida para corte Bs${denomination}.`
    });
  }

  return res.json({
    valid: true,
    message: `Serie valida para corte Bs${denomination}.`
  });
}
